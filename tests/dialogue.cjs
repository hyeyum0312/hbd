...
function schedule(fn, ms, repeat = false) {
  const key = ++id;
  timers.set(key, { fn, at: now + ms, ms, repeat });
  return key;
}
function advance(ms) {
  const end = now + ms;
...
const Component = vm.runInNewContext(logic + "\nComponent", { DCLogic, React, setTimeout: schedule, clearTimeout: (key) => timers.delete(key), setInterval: (fn, ms) => schedule(fn, ms, true), clearInterval: (key) => timers.delete(key) });
const app = new Component();
app.componentDidMount();
function button(label) {
  return app.renderVals().choices.find((b) => b.props.children[0].includes(label));
}
...
assert(button("NEXT"));
const travelNext = button("NEXT");
travelNext.props.onClick();
travelNext.props.onClick();
assert.equal(app.renderVals().dialogue, "");
...
advance(6000);
assert.equal(app.state.endPhase, 2);
advance(10000);
assert.equal(app.state.endPhase, 2);
assert.equal(button("NEXT").props.disabled, false);
click("NEXT");
...
click("BACK");
assert.equal(app.state.ch, "message");
assert.equal(app.state.endPhase, 2);
assert.equal(app.renderVals().dialogue.endsWith("...!"), true);
advance(8000);
assert.equal(app.state.endPhase, 2);
click("BACK");
assert.equal(app.state.endPhase, 1);
click("BACK");
assert.equal(app.state.endPhase, 0);
click("BACK");
assert.equal(app.state.ch, "space");
...
advance(8000);
click("NEXT");
assert.equal(app.state.endPhase, 5);
assert.equal(app.renderVals().dialogue, "");
advance(13000);
...
assert.equal(topDesk.x, bottomDesk.x);
assert(topDesk.y < bottomDesk.y);
memoryTo(5);
...
advance(15000);
assert.equal(app.state.memoryPhase, 9);
const napScene = JSON.stringify(app.renderVals().scene);
click("NEXT");
advance(5000);
assert.equal(app.state.memoryPhase, 10);
...
assert.equal(app.state.endPhase, 3);
assert.equal(app.renderVals().scene.props["data-kiss"], true);
assert(app.renderVals().dialogue.startsWith("지운아 앞으로도 하고 싶은 거 실컷 하고,"));
click("BACK");
assert.equal(app.state.endPhase, 2);
click("BACK");
assert.equal(app.state.endPhase, 1);
...
click("BACK");
assert.equal(app.state.ch, "memory");
assert.equal(app.state.memoryPhase, 15);
assert.equal(app.state.level, 31);
...
assert(template.includes("const GIRL"));
assert(template.includes("◀ BACK"));
...
for (const search of ["", "?sample", "?sample=1", "?other=sample"]) {
  const Variant = vm.runInNewContext(logic + "\nComponent", { DCLogic, React, URLSearchParams, window: { location: { search }, innerWidth: 960 }, setTimeout: schedule, clearTimeout: (key) => timers.delete(key), setInterval: (fn, ms) => schedule(fn, ms, true), clearInterval: (key) => timers.delete(key) });
  const variant = new Variant();
  variant.componentDidMount();
  variant.go("message");
  advance(6000);
  variant
    .renderVals()
    .choices.find((b) => b.props.children[0].includes("NEXT"))
    .props.onClick();
  ...
  variant.back();
  assert.equal(variant.renderVals().dialogue, text);
